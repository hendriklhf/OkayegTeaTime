...
private static string GenerateString()
{
    ...
    result += "</tr>";
    new JsonController().CommandLists.Commands.OrderBy(cmd => cmd.CommandName).ToList().ForEach(cmd =>
    {
    ...
    result += "</tr>";
    new JsonController().CommandLists.AfkCommands.OrderBy(cmd => cmd.CommandName).ToList().ForEach(cmd =>
    {
        result += $"<tr><td>{cmd.CommandName}</td><td><table>";
        cmd.Alias.OrderBy(alias => alias).ToList().ForEach(alias => result += $"<tr><td>{alias}</td></tr>");
        result += "</table></td>";
        for (int i = 0; i <= cmd.Parameter.Count - 1; i++)
        {
...
{
    File.WriteAllText(_readMePath, GenerateString());
}
